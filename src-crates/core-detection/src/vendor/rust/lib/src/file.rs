...
use crate::vendor::content::ContentType;
...
impl FileType {
    pub(crate) fn convert(tensor: ArrayViewD<f32>) -> Vec<FileType> {
        ...
        for view in tensor.view().axis_iter(ndarray::Axis(0)) {
            ...
            let label =
                unsafe { std::mem::transmute::<u32, Label>(best as u32) };
            let inferred_type = label.content_type();
            let config = &crate::model::CONFIG;
            let mut content_type = if score
                < config.thresholds[inferred_type as usize]
            {
                ...
                (overwrite != inferred_type)
                    .then_some((overwrite, OverwriteReason::OverwriteMap))
            };
            if content_type
                .as_ref()
                .is_some_and(|(x, _)| *x == inferred_type)
            {
                content_type = None;
            }
            results.push(FileType::Inferred(InferredType {
                content_type,
                inferred_type,
                score,
            }));
