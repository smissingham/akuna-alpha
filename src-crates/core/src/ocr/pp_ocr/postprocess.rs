...
pub(crate) fn postprocess_recognizer<B: Backend<FloatElem = f32>>(
    ...
    for step in 0..steps {
        ...
        if debug_indices.len() < 32 {
            debug_indices.push((best_index, best_score));
        }
        confidence_sum += best_score;
    }

    if std::env::var_os("AKUNA_OCR_PADDLE_DEBUG").is_some() {
        eprintln!(
            "paddle rec dims=({dim1},{dim2}) classes={classes} steps={steps} dict={} first={debug_indices:?}",
            dictionary.len()
        );
    }
