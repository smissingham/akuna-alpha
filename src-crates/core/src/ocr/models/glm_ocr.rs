...
#[allow(dead_code)]
const GLM_OCR_MAX_TOKENS: usize = 10;
#[allow(dead_code)]
    ...
    B: Backend<FloatElem = f32>,
{
    ...
    fn extract_image_native(
        ...
        for current_position in
            (next_decode_position(&position_ids)..).take(GLM_OCR_MAX_TOKENS)
        {
...
where
    ...
    B::Device: Default,
{
    let files = download_glm_ocr_model(cache_dir.clone()).await?;
...
async fn download_glm_ocr_model(
    ...
    let repo = api.repo(Repo::with_revision(
        GLM_OCR_REPO_ID.to_string(),
        RepoType::Model,
        GLM_OCR_REVISION.to_string(),
    ));

    Ok(GlmOcrFiles {
        config_path: get_repo_file(&repo, "config.json").await?,
        generation_config_path: get_repo_file(&repo, "generation_config.json")
            .await?,
        ...
        .await?,
        tokenizer_path: get_repo_file(&repo, "tokenizer.json").await?,
        weights_path: get_repo_file(&repo, "model.safetensors").await?,
    })
}
