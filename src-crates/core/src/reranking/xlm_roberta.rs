...
#[derive(Module, Debug)]
struct XlmRobertaForSequenceClassification<B: Backend> {
...
pub(crate) struct XlmRobertaRerankerModel<B: Backend> {
    model: XlmRobertaForSequenceClassification<B>,
    tokenizer: Tokenizer,
    pub(crate) variant: XlmRobertaRerankerVariant,
}
...
/// Local filesystem paths to a downloaded Hugging Face model's files.
#[derive(Debug)]
...
pub(crate) async fn download_hf_model(
    ...
    let api = builder
        .build()
        .context("failed to initialize Hugging Face API for reranker model")?;
    let repo = api.model(repo_id.to_string());

    Ok(HfModelFiles {
        config_path: repo.get("config.json").await.with_context(|| {
            format!("failed to fetch reranker config for {repo_id}")
        })?,
        weights_path: repo.get("model.safetensors").await.with_context(
            || format!("failed to fetch reranker weights for {repo_id}"),
        )?,
        tokenizer_path: repo.get("tokenizer.json").await.with_context(
            || format!("failed to fetch reranker tokenizer for {repo_id}"),
        )?,
    })
}
