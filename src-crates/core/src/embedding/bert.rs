...
/// BERT-family embedding checkpoint variants supported by this crate.
...
/// Whether an embedding input is a search query or a stored document.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum EmbeddingInputKind {
    /// Search query input; uses query-routed prompts when supplied.
    Query,
    /// Stored corpus input; uses no prompt unless supplied.
    Document,
}
...
impl BertEmbeddingVariant {
    /// Hugging Face repository identifier for this checkpoint.
...
/// Locally cached Hugging Face files for an embedding checkpoint.
#[derive(Debug)]
pub(crate) struct HfModelFiles {
    /// Path to the downloaded model config JSON.
    ...
    /// Optional path to the sentence-transformers config JSON.
